...
Partial Class Frm_ReponsesListing
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_Reponses.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_ReponsesADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
                'Btn_ADD_Reponses.Attributes.Add("onclick", "javascript:Open_Window('Frm_ReponsesADD.aspx', '_self',500,400); return false;") 
                BindGrid()
            End If
        End If
    End Sub
...
#Region "SECURITE"
    Public Sub SYSTEME_SECURITE()
        Try
            ...
            'CType(Page.Master.FindControl("li_Reponses"), HtmlControl).Attributes.Add("class", "active ")
            'CType(Page.Master.FindControl("i_Reponses"), HtmlControl).Attributes.Add("class", "fa fa-folder-open fa-lg ")
            ...
            CType(Page.Master.FindControl("li_Questions_ReponsesListing2"), HtmlControl).Attributes.Add("class", "active")
            ...
            If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
                ...
                Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                    ...
                    rdgReponses.MasterTableView.Columns.FindByUniqueNameSafe("delete").Visible = _check
                End If
            End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        ...
        Dim _ret As Long = 0
        Try
            objs = Cls_Reponses.SearchAll
            rdgReponses.DataSource = objs
            If _refresh Then
                rdgReponses.DataBind()
            End If
            _ret = objs.Count
...
#Region "RADGRID EVENTS"
    Protected Sub rdgReponses_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgReponses.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgReponses.ExportSettings.ExportOnlyData = True
                rdgReponses.GridLines = GridLines.Both
                rdgReponses.ExportSettings.IgnorePaging = True
                rdgReponses.ExportSettings.OpenInNewWindow = False
                rdgReponses.ExportSettings.FileName = "Liste des Reponses"
                rdgReponses.MasterTableView.Columns(0).Visible = False
                rdgReponses.MasterTableView.ExportToExcel()
            End If
            ...
            Select Case e.CommandName
                Case "delete"
                    ...
                    'User_Connected.Activite_Utilisateur_InRezo("DELETE Reponses ", obj.ID & " - Code:" & obj.Titrerapport & " Prop:", Request.UserHostAddress)
    ...
    Protected Sub rdgReponses_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgReponses.ItemDataBound
        Try
            ...
            If e.Item.ItemType = GridItemType.Item Or e.Item.ItemType = GridItemType.AlternatingItem Then
                ...
                '_lnk.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_ReponsesADD.aspx?ID=" & CLng(_lbl_ID.Text) & "', 750, 400));")
            End If

            If (gridDataItem IsNot Nothing) Then
                Dim item As GridDataItem = gridDataItem
                CType(item.FindControl("lbOrder"), Label).Text = rdgReponses.PageSize * rdgReponses.CurrentPageIndex + (item.RowIndex / 2)
    ...
    Protected Sub rdgReponses_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgReponses.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgReponses.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgReponses.MasterTableView.FilterExpression = String.Empty
            rdgReponses.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
