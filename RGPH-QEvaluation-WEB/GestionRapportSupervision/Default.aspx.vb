...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-API-QEVALUATION"  ' POUR LA PAGE
...
#Region "Load DATA"
    ...
    Private Sub BindGrid_Formulaire(Optional ByVal _refresh As Boolean = True)
    ...
    Private Sub SetAll_DATA_Question_Or_Reponse_From_RadGrid()
        ...
        Dim _JustificationReponseList As New List(Of Cls_JustificationReponses)
        Dim onElementCheck As Boolean = False
        Try
            REM Test pour voir si la case Question est cocher
            REM Parcourt CheckBox du type de Module dans le RadGrid
            For Each lign As GridDataItem In RadGrid1.Items
                Dim CheckBox_FormulaireExercice As CheckBox = CType(lign.FindControl("CheckBox_FormulaireExercice"), CheckBox)
                Dim txt_CodeExercice As TextBox = CType(lign.FindControl("txt_CodeExercice"), TextBox)

                Dim _qList As List(Of Cls_Questions)
                REM Parcourt CheckBoxList Formulaire
                If CheckBox_FormulaireExercice.Checked Then
                    _qList = Cls_Questions.SearchAll_In_QuestionFormulaire_ByIDFormulaire(TypeSafeConversion.NullSafeLong(txt_CodeExercice.Text))
                    If _qList IsNot Nothing OrElse _qList.Count > 0 Then 'Verification pour la liste des questions
                        For Each _quest As Cls_Questions In _qList 'Parcourt de la liste des questions
                            If CBX_Questions.Checked Then
                                _questionList.Add(_quest)
                            ...
                            If CBX_Reponse.Checked Then 'Test pour voir si la case Reponse est cocher
                                REM REPONSES
                                Dim _repList As List(Of Cls_Reponses) = Cls_Reponses.SearchAllBy_CodeQuestion(_quest.codeQuestion)
                                If _repList IsNot Nothing OrElse _repList.Count > 0 Then 'Verification pour la liste des Reponses
                                    For Each _reponse As Cls_Reponses In _repList 'Parcourt de la liste des Reponses
                                        _ReponseList.Add(_reponse)
                                    Next
                                End If
                                REM JUSTIFICATION
                                Dim _justList As List(Of Cls_JustificationReponses) = Cls_JustificationReponses.SearchAllBy_CodeQuestion(_quest.codeQuestion)
                                If _justList IsNot Nothing OrElse _justList.Count > 0 Then 'Verification pour la liste des Cls_JustificationReponses
                                    For Each _Just As Cls_JustificationReponses In _justList 'Parcourt de la liste des Cls_JustificationReponses
                                        _JustificationReponseList.Add(_Just)
                                    Next
                                End If
                            End If
                        Next
            ...
            If _questionList.Count > 0 Then
                onElementCheck = True
                Me.Session([Global].DATA_QUESTION_SESSION) = _questionList
                DIV_Module_QuestionSession.Visible = True
                DIV_Module_QuestionSession.Attributes.Add("onclick", "javascript:OpenWindow('ExportJsonData.aspx?" & [Global].ACTION & "=" & [Global].DATA_QUESTION_SESSION & "'); return false;")
                Literal_QuestionSession.Text = " [ " & _questionList.Count & " ]"
            End If
            ...
            If _JustificationReponseList.Count > 0 Then
                onElementCheck = True
                Me.Session([Global].DATA_JUSTIFICATION_REPONSES_SESSION) = _JustificationReponseList
                DIV_Module_JustificationReponseSession.Visible = True
                DIV_Module_JustificationReponseSession.Attributes.Add("onclick", "javascript:OpenWindow('ExportJsonData.aspx?" & [Global].ACTION & "=" & [Global].DATA_JUSTIFICATION_REPONSES_SESSION & "'); return false;")
                Literal_JustificationReponseSession.Text = "  [ " & _JustificationReponseList.Count & " ]"
            End If
    ...
    Private Sub SetAll_DATA_Module_ET_Formulaire_From_RadGrid()
        Dim _ModuleList As New List(Of Cls_FormulaireExercices)
        Dim _QuestionModuleList As New List(Of Cls_QuestionFormulaireExercice)
        Dim onElementCheck As Boolean = False
        Try
            REM Test pour voir si la case Question est cocher
            REM Parcourt CheckBox du type de FormulaireExercice dans le RadGrid
            For Each lign As GridDataItem In RadGrid1.Items
                Dim CheckBox_FormulaireExercice As CheckBox = CType(lign.FindControl("CheckBox_FormulaireExercice"), CheckBox)
                Dim _CodeExercice = TypeSafeConversion.NullSafeLong(CType(lign.FindControl("txt_CodeExercice"), TextBox).Text)

                Dim _mod As New Cls_FormulaireExercices
                If CheckBox_FormulaireExercice.Checked Then
                    _mod = New Cls_FormulaireExercices(_CodeExercice)
                    If _mod.ID > 0 Then 'Verification pour le FormulaireExercice
                        _ModuleList.Add(_mod)
                        REM recherche des Questions par Module
                        Dim _qmList As List(Of Cls_QuestionFormulaireExercice) = Cls_QuestionFormulaireExercice.SearchAllBy_CodeFormulaireExercice(_mod.CodeExercice)
                        If _qmList IsNot Nothing OrElse _qmList.Count > 0 Then 'Verification pour la liste des Reponses
                            For Each _qmod As Cls_QuestionFormulaireExercice In _qmList 'Parcourt de la liste des Reponses
            ...
            If _ModuleList.Count > 0 Then
                onElementCheck = True
                Me.Session([Global].DATA_FORMULAIRE_SESSION) = _ModuleList
                DIV_Module_ModuleSession.Visible = True
                DIV_Module_ModuleSession.Attributes.Add("onclick", "javascript:OpenWindow('ExportJsonData.aspx?" & [Global].ACTION & "=" & [Global].DATA_FORMULAIRE_SESSION & "'); return false;")
            ...
            If _QuestionModuleList.Count > 0 Then
                onElementCheck = True
                Me.Session([Global].DATA_QUESTION_FORMULAIRE_SESSION) = _QuestionModuleList
                DIV_Question_Module_Session.Visible = True
                DIV_Question_Module_Session.Attributes.Add("onclick", "javascript:OpenWindow('ExportJsonData.aspx?" & [Global].ACTION & "=" & [Global].DATA_QUESTION_FORMULAIRE_SESSION & "'); return false;")
...
#Region "EVENTS CONTROLS"
    ...
    Private Sub CBX_Reponse_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_Questions.CheckedChanged, CBX_Reponse.CheckedChanged
        setValueButon()
    End Sub
