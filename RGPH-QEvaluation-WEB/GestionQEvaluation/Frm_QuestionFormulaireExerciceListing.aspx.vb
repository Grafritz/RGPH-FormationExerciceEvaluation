...
REM  Class Frm_QuestionFormulaireExercice

REM Date:12-Apr-2018 10:06 AM
Imports Microsoft
Imports System.Data
...
Partial Class Frm_QuestionFormulaireExerciceListing
...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-LISTING-QUESTIONFORMULAIREEXERCICE"  ' POUR LA PAGE
    Private Const Btn_Save As String = "Bouton-SAVE-QUESTIONFORMULAIREEXERCICE"       ' POUR LE BOUTON D'ENREGISTREMENT
    Private Const Btn_Edit As String = "Bouton-EDIT-QUESTIONFORMULAIREEXERCICE"       ' POUR LE BOUTON DE MODIFICATION
    Private Const Btn_Delete As String = "Bouton-DELETE-QUESTIONFORMULAIREEXERCICE"   ' POUR LE BOUTON DE SUPPRESSION
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        PAGE_TITLE = " Question Formulaire Exercice"
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_QuestionFormulaireExercice.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_QuestionFormulaireExerciceADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
                'Btn_ADD_QuestionFormulaireExercice.Attributes.Add("onclick", "javascript:Open_Window('Frm_QuestionFormulaireExerciceADD.aspx', '_self',500,400); return false;") 
                BindGrid()
            End If
        End If
    End Sub
...
#Region "SECURITE"
    Public Sub SYSTEME_SECURITE()
        Try
            ...
            'CType(Page.Master.FindControl("li_QuestionFormulaireExercice"), HtmlControl).Attributes.Add("class", "active ")
            'CType(Page.Master.FindControl("i_QuestionFormulaireExercice"), HtmlControl).Attributes.Add("class", "fa fa-folder-open fa-lg ")
            ...
            If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
                ...
                Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                    ...
                    Btn_ADD_QuestionFormulaireExercice.Visible = _check
                    rdgQuestionFormulaireExercice.MasterTableView.Columns.FindByUniqueNameSafe("editer").Visible = _check
                    If Request.QueryString([Global].ACTION) IsNot Nothing Then
                    ...
                    rdgQuestionFormulaireExercice.MasterTableView.Columns.FindByUniqueNameSafe("delete").Visible = _check
                End If
            End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        ...
        Dim _ret As Long = 0
        Try
            objs = Cls_QuestionFormulaireExercice.SearchAll
            rdgQuestionFormulaireExercice.DataSource = objs
            If _refresh Then
                rdgQuestionFormulaireExercice.DataBind()
            End If
            _ret = objs.Count
...
#Region "RADGRID EVENTS"
    Protected Sub rdgQuestionFormulaireExercice_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgQuestionFormulaireExercice.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgQuestionFormulaireExercice.ExportSettings.ExportOnlyData = True
                rdgQuestionFormulaireExercice.GridLines = GridLines.Both
                rdgQuestionFormulaireExercice.ExportSettings.IgnorePaging = True
                rdgQuestionFormulaireExercice.ExportSettings.OpenInNewWindow = False
                rdgQuestionFormulaireExercice.ExportSettings.FileName = "Liste des QuestionFormulaireExercice"
                rdgQuestionFormulaireExercice.MasterTableView.Columns(0).Visible = False
                rdgQuestionFormulaireExercice.MasterTableView.ExportToExcel()
            End If
            ...
            Select Case e.CommandName
                Case "delete"
                    ...
                    'User_Connected.Activite_Utilisateur_InRezo("DELETE QuestionFormulaireExercice ", obj.ID & " - Code:" & obj.Titrerapport & " Prop:", Request.UserHostAddress)
                    MessageToShow([Global].Msg_Information_Supprimee_Avec_Succes, "S")
                    rdgQuestionFormulaireExercice.Rebind()
            End Select
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
    ...
    Protected Sub rdgQuestionFormulaireExercice_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgQuestionFormulaireExercice.ItemDataBound
        Try
            ...
            If e.Item.ItemType = GridItemType.Item Or e.Item.ItemType = GridItemType.AlternatingItem Then
                ...
                '_lnk.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_QuestionFormulaireExerciceADD.aspx?ID=" & CLng(_lbl_ID.Text) & "', 750, 400));")
            End If

            If (gridDataItem IsNot Nothing) Then
                Dim item As GridDataItem = gridDataItem
                CType(item.FindControl("lbOrder"), Label).Text = rdgQuestionFormulaireExercice.PageSize * rdgQuestionFormulaireExercice.CurrentPageIndex + (item.RowIndex / 2)
                ...
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_QuestionFormulaireExerciceADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "&" & [Global].ACTION & "=" & [Global].HideMenuHeader & "',900,650));")
                REM Privilege
    ...
    Protected Sub rdgQuestionFormulaireExercice_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgQuestionFormulaireExercice.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgQuestionFormulaireExercice.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgQuestionFormulaireExercice.MasterTableView.FilterExpression = String.Empty
            rdgQuestionFormulaireExercice.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
