...
REM Date:4/12/2018 10:06:02 AM
Imports Microsoft
Imports System.Data
...
Partial Class Frm_QuestionFormulaireExerciceADD
...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-FORMULAIRE-QUESTIONFORMULAIREEXERCICE"  ' POUR LA PAGE
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                'rbtnAddQuestionFormulaireExercice.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_QuestionFormulaireExerciceADD.aspx', 950, 650)); return false;")
                'BtnADDNew.Attributes.Add("onclick", "javascript:Open_Window('Frm_QuestionFormulaireExerciceADD.aspx', '_self',500,400); return false;") 
                LOAD_ALL_DATA()
            End If
        End If
    End Sub
...
#Region "Load DATA"
    Private Sub LOAD_ALL_DATA()
        FillCombo_CodeFormulaireExercice()
        FillCombo_CodeQuestion()
        LOAD_QUESTIONFORMULAIREEXERCICE()
    End Sub

    Private Sub LOAD_QUESTIONFORMULAIREEXERCICE()
        Try
            If Request.QueryString("ID") IsNot Nothing Then
                Dim _id As Long = TypeSafeConversion.NullSafeLong(Request.QueryString("ID"))
                txt_CodeQuestionFormulaireExercice_Hid.Text = _id
                ...
                If obj.ID > 0 Then
                    ...
                    With obj
                        DDL_CodeFormulaireExercice.SelectedIndex = DDL_CodeFormulaireExercice.Items.IndexOf(DDL_CodeFormulaireExercice.Items.FindByValue(.CodeFormulaireExercice))
                        DDL_CodeQuestion.SelectedIndex = DDL_CodeQuestion.Items.IndexOf(DDL_CodeQuestion.Items.FindByValue(.CodeQuestion))
                        txt_OrdreQuestion.Text = .OrdreQuestion
                        CB_EstDebutQuestion.Checked = .EstDebutQuestion
                    End With
                End If
            Else
    ...
    Private Sub FillCombo_CodeFormulaireExercice()
        Try
            Dim objs1 As List(Of Cls_FormulaireExercices) = Cls_FormulaireExercices.SearchAll
            With DDL_CodeFormulaireExercice
                .DataSource = objs1
                .DataValueField = "ID"
                .DataTextField = "LibelleExercice"
                .DataBind()
...
#Region "METHODES - SAVE"
    Private Sub SAVE_QUESTIONFORMULAIREEXERCICE()
        Try
            Dim _id As Long = TypeSafeConversion.NullSafeLong(txt_CodeQuestionFormulaireExercice_Hid.Text)
            Dim obj As New Cls_QuestionFormulaireExercice(_id)
            With obj
                .CodeFormulaireExercice = DDL_CodeFormulaireExercice.SelectedValue
                .CodeQuestion = DDL_CodeQuestion.SelectedValue
                .OrdreQuestion = txt_OrdreQuestion.Text
                .EstDebutQuestion = CB_EstDebutQuestion.Checked
            End With
            obj.Save(User_Connected.Username)
            REM TRACE UTILUSATEUR / Trace Transaction
            User_Connected.Activite_Utilisateur_InRezo(IIf(_id <= 0, "ADD ", "EDIT ") & " QuestionFormulaireExercice", obj.LogData(obj), Request.UserHostAddress)
            txt_CodeQuestionFormulaireExercice_Hid.Text = obj.ID
            '_message = "Sauvegarde Effectuée"
            MessageToShow([Global].Msg_Enregistrement_Effectue, "S", False)
            'RadAjaxManager1.ResponseScripts.Add("CloseAndRefreshListeQuestionFormulaireExercice();")
...
#Region "EVENTS BUTTON"
    Protected Sub Btn_SaveInfo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btn_SaveInfo.Click
        SAVE_QUESTIONFORMULAIREEXERCICE()
    End Sub
